---
title: 'INFSCI 2595 Fall 2020 Homework: 01'
author: "Shruti Gupta"
date: 'Submission time: August 30, 2020 at 11:00PM EST'
output:
  html_document: default
  pdf_document: default
subtitle: 'Assigned August 25, 2020; Due: August 30, 2020'
---
...
This assignment steps through programming syntax with `ggplot2` and introducing the formula interface with the `lm()` function. The grammar of graphics can help create meaningful visualizations which communicate Exploratory Data Analysis (EDA) findings. Visualizations will provide context to the model results, which might be opague and difficult to interpret. In this assignment you will demonstrate basic syntax, go through a detailed EDA of a common data set, and then begin to work through the syntax of fitting a model.    
...
Load in the packages that you will use for this assignment.  
...
```{r, solution_01a}
  iris %>%
  ggplot(mapping = aes(x = Sepal.Length)) + geom_histogram(bins = 6)
```
...
```{r, solution_01b}
iris %>%    
ggplot(mapping = aes(x = Sepal.Length)) + geom_histogram(bins = 6, color = "black")
...
```{r, solution_01c}
iris %>%    
ggplot(mapping = aes(x = Sepal.Length)) + geom_histogram(bins = 6, color = "navyblue", fill = "gold")
```
...
iris %>%    
ggplot(mapping = aes(x = Sepal.Length)) + geom_histogram(bins = 6, color = "navyblue", fill = "gold", size = 1.55)
```
...
```{r, solution_01e}
iris %>%
ggplot(mapping = aes(x = Sepal.Length)) + geom_histogram(bins = 6, color = "navyblue", fill = "gold", size = 1.55, alpha = 0.5)
```
...
diamonds  %>% tibble::as_tibble() %>%glimpse()
...
```{r, solution_02b}
diamonds %>% 
ggplot(mapping = aes(x = color , y = price)) + geom_boxplot()

```

According to the boxplot, I can observe that as the `color` increases, the median `price` also increases and outliers decreases.
...
```{r, solution_02c}
diamonds %>% 
ggplot(mapping = aes(x = color , y = price)) + geom_boxplot() + facet_wrap(~cut)
```
...
```{r, solution_02d}
diamonds %>% 
ggplot(mapping = aes(x = color , y = price)) + geom_boxplot() + facet_grid(clarity~cut)
```
...
diamonds %>% 
ggplot(mapping = aes(x = color , y = price)) + geom_boxplot(mapping = aes(fill = carat > median(carat))) + facet_grid(clarity~cut) + theme(legend.position = "top")
...
Problem 2b, plots is between `price` of the diamonds with respect to `color`. In this figure, it appears that the diamonds with `color` J have high median price and the diamonds with color D and E have lower median price.

Problem 2f, shows the `price` with `carat`. The color D has the highest median price over the other colors. For the diamonds with carat values above the median value, the diamonds with colors D and E have median prices greater than the median prices of the other diamond colors which is opposite to the problem 2b.

So, in conclusion, it is important to see `carat` and `color` while buying diamonds
...
```{r, solution_03e}
ggplot(data = mpg,mapping = aes(x = displ, y = hwy)) + 
geom_point() + geom_smooth(method = lm, formula = y ~ x, mapping = aes(color = drv))
```
...
We could use the `summary()` function to inspect the results, but instead I want you to visualize the coefficience estimates and confidence intervals. To do so, you will use the `coefplot()` function from the `coefplot` package. If you have not downloaded and installed `coefplot` please do so now.  
...
```
`disp` variable is negative and it is "significant".
...
```
Yes, all variables are "significant". 3 coefficients `drvr` , `drvf` and `displ` are displayed
